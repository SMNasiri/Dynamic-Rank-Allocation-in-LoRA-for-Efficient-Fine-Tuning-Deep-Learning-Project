...
def summary_plot(summary_csv: Path, out: Path):
    ...
    metrics = ["accuracy", "training_seconds", "peak_gpu_allocated_mb"]
    methods = sorted(set(r["method"] for r in rows))
    means = defaultdict(dict)
    stds = defaultdict(dict)
    for metric in metrics:
        for method in methods:
            vals = np.array([float(r[metric]) for r in rows if r["method"] == method], dtype=float)
            means[metric][method] = vals.mean()
            stds[metric][method] = vals.std(ddof=1) if len(vals) > 1 else 0.0

    # One figure per metric so report figures remain readable.
